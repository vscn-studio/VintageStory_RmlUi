...
class PackagingTests(unittest.TestCase):
    def setUp(self):
        ...
        self.write_json("src/VSRmlUi/modinfo.json", {"version": "1.0.3"})
        self.write_json("artifacts/managed/manifest.json", {"version": "1.0.3", "bridgeSource": build.bridge_hash(), "sha256": hashlib.sha256(b"test fixture only").hexdigest()})
    ...
    def add_example(self):
        ...
        self.write_json(staged / "modinfo.json", {"modid": "vsrmluiexample", "version": "1.0.3"})
    ...
    def test_single_complete_archive_only(self):
        ...
        self.assertEqual([p.name for p in archives], ["vsrmlui_1.0.3.zip"])
        with zipfile.ZipFile(archives[0]) as z:
            self.assertIsNone(z.testzip())
            self.assertEqual(json.loads(z.read("modinfo.json"))["version"], "1.0.3")
    ...
    def test_fonts_are_rejected_before_replacing_release(self):
        self.package()
        release = self.root / "artifacts/vsrmlui_1.0.3.zip"
    ...
    def test_wrong_managed_version_is_rejected(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "does not match 1.0.3"):
            self.package()
    ...
    def test_windows_linux_release_without_mac(self):
        self.package()
        with zipfile.ZipFile(self.root / "artifacts/vsrmlui_1.0.3.zip") as z:
    ...
    def test_input_diagnostics_archive_is_separate(self):
        self.add_example()
        self.package()
        self.assertEqual(sorted(p.name for p in (self.root / "artifacts").glob("*.zip")), ["vsrmlui-test_1.0.3.zip", "vsrmlui_1.0.3.zip"])
        with zipfile.ZipFile(self.root / "artifacts/vsrmlui-test_1.0.3.zip") as z:
