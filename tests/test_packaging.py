...
class PackagingTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("native/CMakeLists.txt", "native/bridge.h", "native/bridge.cpp", "src/VSRmlUi/assets/test.txt", "licenses/MIT", "LICENSE", "README.md", "VALIDATION.md", "PLATFORMS.zh-CN.md", "DIRECTOR_MIGRATION_GAPS.zh-CN.md", "artifacts/managed/VSRmlUi.dll", "artifacts/managed/VSRmlUi.xml"):
            p = self.root / name
            p.parent.mkdir(parents=True, exist_ok=True)
            p.write_bytes(b"test fixture only")
        (self.root / "build").mkdir()
        self.write_json("src/VSRmlUi/modinfo.json", {"version": "1.0.0"})
        self.write_json("artifacts/managed/manifest.json", {"version": "1.0.0", "sha256": hashlib.sha256(b"test fixture only").hexdigest()})
    ...
    def test_single_complete_archive_only(self):
        ...
        self.assertEqual([p.name for p in archives], ["vsrmlui_1.0.0.zip"])
    ...
    def test_wrong_managed_version_is_rejected(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "does not match 1.0.0"):
            self.package()

    def test_failed_update_preserves_existing_release(self):
        self.package()
        release = self.root / "artifacts/vsrmlui_1.0.0.zip"
